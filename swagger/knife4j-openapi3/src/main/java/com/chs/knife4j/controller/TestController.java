...
public class TestController {
    ...
    @Operation(summary = "map类型入参3(使用动态请求参数注解)", description = "map类型入参接口")
    ...
    @io.swagger.v3.oas.annotations.parameters.RequestBody(content = @Content(schema = @Schema(implementation = Student.class)))
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "成功", content = @Content(schema = @Schema(implementation = Student.class)))
    })
    // @MapResponse(responseCode = "200", description = "成功", rawType = Result.class, typeArgs = Student.class)
    @Operation(summary = "map类型入参4(使用自定义注解)", description = "map类型入参接口")
    @PostMapping("/test_map_params4")
    public Result<Map<String,Object>> test_map_params4(@RequestBody @MapDescription Map<String,String> params){
        return Result.success(null);
    }

    @Operation(summary = "map类型入参5(使用实体类2)", description = "map类型入参接口")
    @PostMapping("/test_map_params5")
    public Result<Map<String,Object>> test_map_params5(@RequestBody User params){
        return Result.success(null);
    }
