...
<div class="right">
        ...
        <div class="providerAdd">
          ...
          <input type="hidden" value="${bill.billCode}" id="olderbillCode">
          <p>${message}</p>
          <form id="billForm" name="billForm" method="post" action="${pageContext.request.contextPath }/billSave.do">
                ...
                <div>
                    ...
                    <input type="text" name="totalPrice" id="totalPrice" value="${bill.totalPrice }" οnkeyup="value=value.replace(/[^\d.]/g,'')"> 
					<font color="red"></font>
                </div>
                ...
                <div>
					...
					<select name="providerId" id="providerId" style="text-align: center;">
							<c:forEach items="${proList}" var="prodiver">
								<c:choose>
									<c:when test="${prodiver.id==bill.providerId}">
										<option value="${prodiver.id}" selected="selected">${provider.proName}</option>
									</c:when>
									<c:otherwise>
										<option value="${prodiver.id}">${prodiver.proName}</option>
									</c:otherwise>
								</c:choose>
							</c:forEach>
		        	</select>
